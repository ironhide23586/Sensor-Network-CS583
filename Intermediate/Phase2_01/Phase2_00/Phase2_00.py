...
def makePreds(window_, preds, betas, timeIdx, dType):
    ...
    z = poly.fit_transform(preds.reshape(-1, 1))
    return np.sum(np.multiply(betas, z), axis=1)
...
def predict_variance_inf_daylvl(budget, train_means, train_vars, betas, dType):
    window_ = None
    i = 1
...
if __name__ == "__main__":
    ...
    tmp = np.sort(np.array(hum_train_means_daylvl.keys()))
    tmp = tmp[1:]
    ...
    for budget in budgets:
        ...
        hum_preds_win_hrlvl, hum_mean_err_win_hrlvl = predict_window_inf_hrlvl(budget, hum_train_means_hrlvl, betas_hum_hrlvl, 'humidity')
        temp_preds_win_hrlvl, temp_mean_err_win_hrlvl = predict_window_inf_hrlvl(budget, temp_train_means_hrlvl, betas_temp_hrlvl, 'temperature')

        hum_preds_var_hrlvl, hum_mean_err_var_hrlvl = predict_variance_inf_hrlvl(budget, hum_train_means_hrlvl, hum_train_vars_daylvl, betas_hum_hrlvl, 'humidity')
        temp_preds_var_hrlvl, temp_mean_err_var_hrlvl = predict_variance_inf_hrlvl(budget, temp_train_means_hrlvl, hum_train_vars_daylvl, betas_temp_hrlvl, 'temperature')
        

        betas_hum_daylvl = get_betas_daylvl(hum_train_data)
        betas_temp_daylvl = get_betas_daylvl(temp_train_data)

        hum_preds_win_daylvl, hum_mean_err_win_daylvl = predict_window_inf_daylvl(budget, hum_train_means_hrlvl, betas_hum_daylvl, 'humidity')
        temp_preds_win_daylvl, temp_mean_err_win_daylvl = predict_window_inf_daylvl(budget, temp_train_means_hrlvl, betas_temp_daylvl, 'temperature')

        hum_preds_var_daylvl, hum_mean_err_var_daylvl = predict_variance_inf_daylvl(budget, hum_train_means_hrlvl, hum_train_vars_daylvl, betas_hum_daylvl, 'humidity')
        temp_preds_var_daylvl, temp_mean_err_var_daylvl = predict_variance_inf_daylvl(budget, temp_train_means_hrlvl, hum_train_vars_daylvl, betas_temp_daylvl, 'temperature')
        ...
        print hum_mean_err_win_daylvl, temp_mean_err_win_daylvl 
        print hum_mean_err_var_daylvl, temp_mean_err_var_daylvl, '\n' 

        print hum_mean_err_win_phase1, temp_mean_err_win_phase1
        print hum_mean_err_var_phase1, temp_mean_err_var_phase1

        print '------------------------------------------------\n'

        write_data(hum_preds_win_hrlvl, str('results/humidity/h-w' + str(budget) + '.csv'))
        write_data(hum_preds_win_daylvl, str('results/humidity/d-w' + str(budget) + '.csv'))
        write_data(hum_preds_var_hrlvl, str('results/humidity/h-v' + str(budget) + '.csv'))
        write_data(hum_preds_var_daylvl, str('results/humidity/d-v' + str(budget) + '.csv'))

        write_data(temp_preds_win_hrlvl, str('results/temperature/h-w' + str(budget) + '.csv'))
        write_data(temp_preds_win_daylvl, str('results/temperature/d-w' + str(budget) + '.csv'))
        write_data(temp_preds_var_hrlvl, str('results/temperature/h-v' + str(budget) + '.csv'))
        write_data(temp_preds_var_daylvl, str('results/temperature/d-v' + str(budget) + '.csv'))

        """Plotting Histograms."""

        plt.xticks(range(6), ['Phase1-window', 'Phase1-variance', 'Phase2-h-window', 'Phase2-h-variance', 'Phase2-d-window', 'Phase2-d-variance'])
        plt.bar(range(6), [hum_mean_err_win_phase1, hum_mean_err_var_phase1, hum_mean_err_win_hrlvl, hum_mean_err_var_hrlvl, hum_mean_err_win_daylvl, hum_mean_err_var_daylvl] , align='center', alpha=0.4)
        plt.xlabel('$Experiments$')
        plt.ylabel('$Mean Absolute Error$')
        plt.title("Error vs Aproaches with Budget=" + str(budget))
        plt.show()

        plt.xticks(range(4), ['Phase1-window', 'Phase1-variance', 'Phase2-h-window', 'Phase2-h-variance'])
        plt.bar(range(4), [hum_mean_err_win_phase1, hum_mean_err_var_phase1, hum_mean_err_win_hrlvl, hum_mean_err_var_hrlvl] , align='center', alpha=0.4)
        plt.xlabel('$Experiments$')
        plt.ylabel('$Mean Absolute Error$')
        plt.title("Error vs Aproaches with Budget=" + str(budget) + "\n(Day Level experiment observations removed due to high error)")
        plt.show()
